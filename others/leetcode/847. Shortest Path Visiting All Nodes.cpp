...
public:
    ...
    int shortestPathLength(vector<vector<int>>& graph) {
        ...
        // we don't know which node will give us shortest path so intially for 
        // all nodes we will store in our queue
        for(int i = 0; i < n; i++)
        {
        ...
        while(q.empty() == false) // until queue is not empty
        {
            ...
            // if mask value becomes all, that means we have visited all of our nodes, 
            // so from here return cost - 1
            if(curr.mask == all) 
            {
                return curr.cost - 1;
            }
...
When we see our constraint i.e 1 <= n <= 12, then it gives us an idea that to find the
shortest path we may are able to explore all possibilites and all of those possibilities 
we find the shortest path.
Suppose if there are n nodes, then how many total possiilities we have, we 
have 2 ^ n possible paths for n nodes.

Whenever we encounter a problem to find the shortest path in graph, 
we will apply bfs to solve it because bfs will gurantee us to be 
the shortest path.
Now, when we know that we have to explore all possibility of a graph, 
then we need some variables and data structure to avoid repetative 
paths and to know whether we explore all other nodes from this current node.
...
Set data structure - It tells us which path we have already visited. 
It warn us to not to travel the repeative paths.
...
So, How we will make changes in our bit, so that we can say that okay if 
we travel from node 2 to node 3
then according to this our bit changes.

So see, by using bit what we are trying to say, we are trying to say that we 
make 1 to the position of node.
...
But also since we now visit to our node 3 then definately we have to set our 
position 3 i.e to make it 1.
...
Since, We don't not know on starting which node we get our minimum path so 
intially for nodes we store mask and values in our queue.
...
now if we convert 111 to decimal then it comes out to be
 7 i.e (2 ^ 3 - 1) (read it as power of 2 of three nodes minus 1)
            ↑

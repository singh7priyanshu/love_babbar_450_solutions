...
a 2D integer array tasks, where tasks[i] = 
[enqueueTimei, processingTimei] means that the i​​​​​​th​​​​ task will 
be available to process at enqueueTimei and will take 
processingTimei to finish processing.

You have a single-threaded CPU that can process at most one task 
at a time and will act in the following way:
...
If the CPU is idle and there are available tasks, the CPU will 
    choose the one with the shortest processing time. If multiple 
    tasks have the same shortest processing time, it will choose 
...
public:
    vector<int> getOrder(vector<vector<int>>& tasks){        
        ...
        for(int i=0;i<n;i++){
            v.push_back({tasks[i][0],tasks[i][1],i});// (enqueTime,processing Time,index)
        }
        sort(v.begin(),v.end());
        long long t=v[0][0];
        int i=0;
        ...
        while(!pq.empty()){
             ...
             t+=temp.first;
             if(i<n&&t<v[i][0]&&pq.empty()) t=v[i][0];
             while(i<n&&v[i][0]<=t){
                  pq.push({v[i][1],v[i][2]});
                  i++;
             }   
